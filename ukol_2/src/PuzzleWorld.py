...
class PuzzleWorld(object):
    ...
    def _solve_given_puzzle(self, start_state):
        ...
        while current_state != self.goal_state:
            ...
            cur_h_value = self.heuristics.calculate_value(self.h_type, current_state)
            # If yes, another step in solution was found.
            if best_state['h_value'] <= cur_h_value:
    ...
    def _create_states_from_moves(self, current_state, possible_moves):
        ...
        """
        new_states = []
        zero_tile = current_state[0]
        for moved_tile in possible_moves:
            new_zero = moved_tile
            new_move_tile = zero_tile
            new_state = current_state.copy()
            new_state[0] = new_zero
            new_state[self._get_tile_name_by_coordinates(current_state, moved_tile)] = new_move_tile
    ...
    def _find_best_state(self, new_states):
        ...
        min_h = float("inf")
        min_states = []
        # Get states with lowest h value.
        ...
        for state in min_states:
            if state['moved_tile'] == self.solution_path[-1][0]:
                continue
            else:
                return state
    ...
    def _get_valid_states(self, new_states):
        # The state cannot be same as start state.
        valid_states = []
        for state in new_states:
